...
-   The difference in sample mean GPA's is `r round(abs(diff(avg_gpa_sex$mean_gpa)), 3)`.

-   I would say the difference is statistically significant using the common p value of 0.05 but we would have to use a t-test to find out.
:::

::: {.callout-note icon="false" title="Problem 2"}
Generate a data visualization that displays the GPA's of the two groups. Be sure to include a title and label your axes.
:::
...
ggplot(sat_gpa, aes(x = sex, y = gpa_fy)) +
  geom_boxplot() +
  labs(title = "Distribution of Freshman GPA's by Sex",
       x = "Sex",
       y = "Freshman GPA")
...
-   The size of the “shuffled” (permuted) sample in each replicate is `r nrow(sat_gpa)`. This is the same as the size of the original dataset since permutation involves shuffling among the give number of reps and not adding or removing any data.
:::
...
```{r}
# Type your code and comments inside the code chunk
num_shuffles <- max(gpas_in_null_world$replicate)
num_shuffles
```

-   We did a different "shuffle" (permute) here to the sample `r num_shuffles`.

-   There are `r nrow(gpas_in_null_world)` rows in the `gpas_in_null_world` data frame.
:::
...
-   There are `r nrow(gpa_diff_under_null)` many rows in the `gpa_diff_under_null` data frame because the number of rows is equal to the number of permutations  you perform.
:::
...
The mean GPA scores for females in our sample ($\bar{x}_{f}$ = `r avg_gpa_sex$mean_gpa[avg_gpa_sex$sex == "Female"]`) was greater than that of males ($\bar{x}_{m}$ = `r avg_gpa_sex$mean_gpa[avg_gpa_sex$sex == "Male"]`). This difference **was** statistically significant at $\alpha = 0.05$, (p = p = 0.001 or 0.0025). Given this p-value, I **would** reject the Null hypothesis and **find evidence** that **females** have higher GPAs than **males** at the population level.
:::
...
-   The null hypothesis is that The population mean total SAT score for students with a 'low' hs gpa is equal to the population mean total SAT score for students with a 'high' hs gpa.

-   $$\begin{aligned} H_0:&\mu_{\text{high}} - \mu_{\text{low}} = 0 \end{aligned}$$
...
obs_diff_sat_hs_gpa <- diff(avg_sat_gpa$mean_sat_total)
round(abs(obs_diff_sat_hs_gpa), 3)
...
```{r}
# Type your code and comments inside the code chunk
sat_scores_in_null_world <- sat_gpa %>% 
  specify(sat_total ~ gpa_hs) %>% 
  hypothesize(null = "independence") %>% 
  generate(reps = 2000, type = "permute")
kable(head(sat_scores_in_null_world))
```
:::
...
```{r}
# Type your code and comments inside the code chunk
sat_diff_under_null <- sat_scores_in_null_world %>%
...
ggplot(sat_diff_under_null, aes(x = stat)) +
  ...
  geom_vline(aes(xintercept = obs_diff_sat_hs_gpa), color = "red", linetype = "dashed") +
  labs(title = "Null Distribution of Differences in SAT Scores",
...
```{r}
# Type your code and comments inside the code chunk
p_value <- mean(sat_diff_under_null$stat >= obs_diff_sat_hs_gpa)
p_value
```
...
-   A p-value of `r p_value` means that the hypothesis test failed to reject the null hypothesis and there is no statistical significance.
:::
...
```{r}
# Type your code and comments inside the code chunk
bootstrap_samples <- sat_gpa %>%
  specify(sat_total ~ gpa_hs) %>%
  generate(reps = 2000, type = "bootstrap")

bootstrap_differences <- bootstrap_samples %>%
  calculate(stat = "diff in means", order = c("high", "low"))

ci_diff_sat_scores <- bootstrap_differences %>%
  get_ci(level = 0.95, type = "percentile")

ci_diff_sat_scores
```

-   The confidence interval is: `r ci_diff_sat_scores`.
:::
...
obs_t_stat <- t.test(sat_total ~ gpa_hs, data = sat_gpa)$statistic
obs_t_stat
